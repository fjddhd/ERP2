...
public interface ProductionProcessRepository
	 ...
	 * @return
	 */
	@Query(value = "SELECT * FROM t_production_process WHERE id NOT IN (SELECT id FROM t_production_process WHERE issue_state LIKE '%完成%' OR issue_state LIKE '%未下发%') AND ji_tai_id = ?1", nativeQuery = true)
	 ...
	 */
	@Modifying
	@Query(value = "UPDATE t_production_process SET accomplish_state = ?1 , state = ?1 WHERE sale_list_product_id = ?2", nativeQuery = true)
